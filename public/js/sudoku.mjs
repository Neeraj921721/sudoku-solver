...
function getBoxValidation(boardInputElements) {
    ...
    for (let startIndex of startIndexArray) {
        const boxArray = [0, 1, 2, 9, 10, 11, 18, 19, 20].map(
            (numToAdd) => {
...
function removeUndefined(array) {
    return array.filter(
        (element) => !isNaN(element) && element != undefined
    );
}

function isArrayUnique(arr) {
    const f_arr = removeUndefined(arr);
    if (f_arr.length == 0) return true;
...
function recursivelySolveTheBoard(boardArray) {
    ...
    // if no solution exist for the current unsolved index, set it as undefined and return false
    boardArray[nextUnSolvedCellIndex] = undefined;
    return false;
}

function getNextCellAndPossibilities(boardArray) {
    for (let i = 0; i < boardArray.length; i++) {
        if (isNaN(boardArray[i]) || boardArray[i] == undefined) {
            const existingValues = getAllIntersectionsRowColBox(
                boardArray,
                i
            );
            const choices = [1, 2, 3, 4, 5, 6, 7, 8, 9].filter(function (
                num
            ) {
...
function getColumn(boardArray, i) {
    const chunkSize = 9;
    let startingElement = Math.floor(i % 9);
    const startIndexArray = [0, 1, 2, 3, 4, 5, 6, 7, 8];
    return startIndexArray.map(function (num) {
        return boardArray[startingElement + num * chunkSize];
    });
}
...
export { solveSudoku, isValidBoard };